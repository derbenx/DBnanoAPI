...
;Todo
; display size of dropped image.
;  display the number of tasks it has too.
;  double click to select a new image
;  arrow keys change img selection.
...
Tab.UseTab(1)
LV_Images := MyGui.Add("ListView", "w" . imgw . " h" . imgh, ["#","Image", "Path"])
...
LV_Tasks := MyGui.Add("ListView", "x30 y250 w" . imgw*2+10 . " h140", ["Img", "Agent", "Res", "Ratio", "Status", "Cost ($)", "JobID", "Prompt"])
...
SaveCSV(*) {
    ...
    try {
        ...
        for filePath, tasks in ImageTaskMap {
            ; Write the image path line: img, index, path
            fileObj.WriteLine("img," . imgIndex . "," . filePath)
            ...
            for task in tasks {
                ...
                line := "tsk," . imgIndex . "," . task.Size . "," . task.Agent . "," . task.Ratio . "," . cleanPrompt . "," . cleanNeg . "," . task.Format
...
LoadCSV(*) {
        ...
        Loop Read, loadPath {
            ...
            if (parts[1] == "img") {
                ...
                if tempImgMap.Has(parentIdx) {
                    parentPath := tempImgMap[parentIdx]
    ...
    Mode: Radio_Batch.Value ? "Batch" : "Immediate"
                    ...
                    ImageTaskMap[parentPath].Push(newTask)
                }
            }
        }
        ; UI Refresh logic [cite: 23, 24, 25]
...
ShowTaskForm(*) {
    ...
    ; // 1. Identify which image is currently selected in the Image List
    selectedImageRow := LV_Images.GetNext(0, "Focused")
    if (selectedImageRow == 0) {
        MsgBox "Please select an image on the left first!" 
        return
    }

    ; // Get the ID (#) and Path from the Image List
    imgIDText := LV_Images.GetText(selectedImageRow, 1) ; // Column 1 is the #
    imgID := Integer(imgIDText) ; // Convert to integer for Map lookup
    currentImgPath := LV_Images.GetText(selectedImageRow, 3) ; // Column 3 is Full Path
    ...
    pVal := ""
    nVal := ""
    ; // 3. If Editing, retrieve existing data from the Integer-based Map
    if (isEdit && selectedTaskRow > 0) {
        ; // Calculate Local Index: How many tasks for THIS imgID exist up to this row?
        Loop selectedTaskRow {
            if (LV_Tasks.GetText(A_Index, 1) == imgIDText) {
                localIdx++
            }
        }

        ; // Access the Map using the Integer ID and the calculated Local Index
        if ImageTaskMap.Has(imgID) && ImageTaskMap[imgID].Length >= localIdx {
            task := ImageTaskMap[imgID][localIdx]
            pVal := task.Prompt
            nVal := task.NegativePrompt
            
            ; // Find the dropdown index for the Tier
            ...
            ; // Find the dropdown index for the Ratio
            for i, val in ratioList {
                if (val == task.Ratio)
                    ratioChoice := i
            }
                
            ; // Find the dropdown index for the Format
    ...
    ; // Button Event
    btn.OnEvent("Click", (*) => (
        SubmitTaskWithExtras(g.Submit(), isEdit, localIdx), ; // Use the calculated localIdx
        MyGui.Opt("-Disabled"), 
        g.Destroy(),
        MyGui.Show()
    ))
...
SubmitTaskWithExtras(Data, isEdit := false, editIndex := 0) {
    ; // Extract Agent and Size from the Tier string (e.g., "Nano Pro 4K")
    RegExMatch(Data.Tier, "(.*)\s(\d+K)", &match) ; // [cite: 100]
    
    ; // 1. Get the Image ID (#) from the first column of the selected image
    selectedImageRow := LV_Images.GetNext(0, "Focused")
    if (selectedImageRow == 0)
        return
    imgID := Integer(LV_Images.GetText(selectedImageRow, 1)) ; // 
    ...
    newTask := {
        ...
        Format: Data.Format,
        Status: "Pending",
        ; // Explicitly store the path inside the task for the runner to find [cite: 102, 113]
        SourcePath: LV_Images.GetText(selectedImageRow, 3), 
        Cost: CalculateCost(match[1], match[2])
    }

    ; // 3. Store in the Map using the Integer ID as the key
    if (isEdit && editIndex > 0) {
        ImageTaskMap[imgID][editIndex] := newTask ; // [cite: 103]
    } else {
    ...
    UpdateTotalDisplay() ; // [cite: 105]
    RefreshTaskTable()   ; // [cite: 108]
    UpdateButtonStates() ; // [cite: 106]
}
...
RefreshTaskTable() {
    ...
    for imgID, tasks in ImageTaskMap {
        for i, t in tasks {
            ...
            LV_Tasks.Add(, imgID, t.Agent, t.Size, t.Ratio, t.Status, Format("{:.3f}", t.Cost), t.Prompt) ;[cite: 304]
        }
    }
    LV_Tasks.ModifyCol()
}
...
Gui_DropFiles(GuiObj, GuiCtrlObj, FileArray, X, Y) {
    for i, file in FileArray {
        SplitPath file, &fn
        ix := LV_Images.GetCount() + 1
        LV_Images.Add(, ix, fn, file) ; Store filename in Col 1, Full Path in Col 2
...
ProcessNextTask() {
    ...
    imgIDText := LV_Tasks.GetText(CurrentBatchIndex, 1)
    if (imgIDText == "")
      return
    imgID := Integer(imgIDText)
    ...
    try {
        ...
        ; // SUCCESS: Passing exactly 3 parameters as defined above
...
CreateJsonPayload(taskObj, taskImagePath) {
    ...
    ;ModelLog.Value .= "`nStuff: " . TaskObj.Ratio . " " . TaskObj.Size
    ;SendMessage(0x0115, 7, 0, ModelLog.Hwnd, "A") ; WM_VSCROLL = 0x0115, SB_BOTTOM = 7 
    ;return
    ...
    return Format(template, fullPrompt, mime, b64, encourage, icfg)
}
...
$Del:: {
    ...
    if (FocusedCtrl == LV_Images) {
        ...
        if (Row) {
            ; // Use Column 3 for the path (shifted due to the new # column)
            imgIDText := LV_Images.GetText(Row, 1) 
            imgID := Integer(imgIDText)
            if ImageTaskMap.Has(imgID) {
              ImageTaskMap.Delete(imgID) ; //
            }
            
            LV_Images.Delete(Row) ; // Remove from UI
            
            ; // IMPORTANT: Refresh the global task table to remove orphaned tasks
            RefreshTaskTable() 
            ...
            if (LV_Images.GetCount() > 0) {
                ...
                ImageListClick(LV_Images, NewRow) 
            }
        }
    }
        ...
        if (Row) {
            ; // In a global view, we need to find which image this task belongs to
            ; // Read the Image Index from Task Column 1
            targetImgIdx := LV_Tasks.GetText(Row, 1) ; 
            
            ; // Find the path for that image index in the Image List
            targetPath := ""
            Loop LV_Images.GetCount() {
                if (LV_Images.GetText(A_Index, 1) == targetImgIdx) {
                    targetPath := LV_Images.GetText(A_Index, 3) ; 
                    break
                }
            }

            ; // Remove from the specific sub-array in the Map
            ; // Note: This assumes tasks are displayed in the order they exist in the Map
            if (targetPath != "" && ImageTaskMap.Has(targetPath)) {
                ; // Simple logic: This only works if the list isn't sorted
                ; // Better logic: store a unique TaskID in a hidden column
                ImageTaskMap[targetPath].RemoveAt(1) ; // Placeholder logic
            }
