...
global negVal := "distorted faces, blurry, distorted, low quality, text, watermarks, missing or extra limbs, deformities, floating people or objects"
...
tier := MyGui.Add("DropDownList", "x+10 disabled", ["Nano Flash 1K", "Nano Pro 1K", "Nano Pro 2K", "Nano Pro 4K","Imagen 2K","Imagen Ultra 2K"])
...
CreateNewTask(*) {
    ...
    ; 3. Grab the ratio from the first image only (as discussed)
    masterRatio := "1:1"
    firstPath := selectedPaths[1]
    if FileExist(firstPath) {
        ; ... (Keep your existing Shell.Application ratio detection logic here)
        ; Set masterRatio based on the firstPath dimensions
    }
