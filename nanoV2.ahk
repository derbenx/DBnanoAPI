...
;Todo
; add a 'noimg' for generating an image (not editing)
;  pricing for 'noimg' ?
; tag multiple images to merge into one.
...
SaveCSV(*) {
    ...
    try {
        ...
        Loop LV_Images.GetCount() {
            imgID := Integer(LV_Images.GetText(A_Index, 1))
        ...
        Loop Read, loadPath {
            ...
            if (parts[1] == "img") {
                ...
                if tempImgMap.Has(parentIdx) {
                    ...
                    Loop LV_Images.GetCount() {
                        if (Integer(LV_Images.GetText(A_Index, 1)) == currentIx) {
...
ShowTaskForm(*) {
    ...
    currentImgPath := LV_Images.GetText(selectedImageRow, 5) ; // Column 5 is Full Path
    ...
    if (isEdit && selectedTaskRow > 0) {
        ; // Use the hidden TaskIdx column to get the actual array position
        localIdx := Integer(LV_Tasks.GetText(selectedTaskRow, 8))

        ; // Access the Map using the Integer ID and the retrieved Index
...
SubmitTaskWithExtras(Data, isEdit := false, editIndex := 0) {
    ...
    newTask := {
        ...
        SourcePath: LV_Images.GetText(selectedImageRow, 5),
    ...
    if (isEdit && editIndex > 0) {
        ...
        ; Update task count in LV_Images
        Loop LV_Images.GetCount() {
            if (Integer(LV_Images.GetText(A_Index, 1)) == imgID) {
                LV_Images.Modify(A_Index, "", , , ImageTaskMap[imgID].Length)
                break
            }
        }
    }
...
ImageListDoubleClick(LV, RowNum) {
    ...
    imgID := Integer(LV.GetText(RowNum, 1))

    ; Update ListView: Col 2 (MBs), Col 4 (Image), Col 5 (Path)
    LV.Modify(RowNum, "", , sizeMB, , fn, newPath)

    ; Update SourcePath in ImageTaskMap
    if ImageTaskMap.Has(imgID) {
        for t in ImageTaskMap[imgID] {
...
$Del:: {
    ...
    if (FocusedCtrl == LV_Images) {
        ...
        if (Row) {
            imgID := Integer(LV_Images.GetText(Row, 1))
            if ImageTaskMap.Has(imgID) {
        ...
        if (Row) {
            targetImgID := Integer(LV_Tasks.GetText(Row, 1))
            ...
            if ImageTaskMap.Has(targetImgID) {
                if (taskIdx > 0 && taskIdx <= ImageTaskMap[targetImgID].Length) {
                    ...
                    ; Update task count in LV_Images: Col 3 (tasks)
                    Loop LV_Images.GetCount() {
                        if (Integer(LV_Images.GetText(A_Index, 1)) == targetImgID) {
                            LV_Images.Modify(A_Index, "", , , ImageTaskMap[targetImgID].Length)
                            break
                        }
                    }
                }
